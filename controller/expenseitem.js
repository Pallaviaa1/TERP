...
const getAllExpenseItems = async (req, res) => {
	try {
		const [data] = await db.query(
			`SELECT Expense_Items.*, dropdown_type.type_name_en, dropdown_type.type_name_th, Chart_of_Accounts.Name as account_name  FROM Expense_Items 
			LEFT JOIN Chart_of_Accounts ON Chart_of_Accounts.Account = Expense_Items.chart_of_accounts 
			LEFT JOIN dropdown_type ON dropdown_type.type_id = Expense_Items.Type`,
		)
		res.status(200).json({
